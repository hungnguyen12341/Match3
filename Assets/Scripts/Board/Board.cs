...
public class Board
{
    ...
    internal void Shuffle()
    {
        for (int x = 0; x < boardSizeX; x++)
        {
            for (int y = 0; y < boardSizeY; y++)
            {
                ...
                Cell temp = m_cells[x, y];
                m_cells[x, y] = m_cells[rndX, rndY];
                m_cells[rndX, rndY] = temp;

                m_cells[x, y].ApplyItemMoveToPosition();
                m_cells[rndX, rndY].ApplyItemMoveToPosition();
            }
        }
    }
    ...
    internal void FillGapsWithNewItems()
    {
        for (int x = 0; x < boardSizeX; x++)
        {
            int emptyCellsCount = 0; // Count the number of empty cells in each column.
            for (int y = 0; y < boardSizeY; y++)
            {
                Cell cell = m_cells[x, y];
                if (cell.IsEmpty)
                ...
                {
                    // If there are empty cells below this cell, move the item down.
                    Cell targetCell = m_cells[x, y - emptyCellsCount];
                    targetCell.Assign(cell.Item);
                    cell.Free();
                    targetCell.ApplyItemPosition(true);
                }
            }

            // Fill the top empty cells with new items.
            for (int y = boardSizeY - emptyCellsCount; y < boardSizeY; y++)
            {
                Cell cell = m_cells[x, y];
                NormalItem.eNormalType newItemType = GetNewItemType(GetSurroundingItemTypes(x, y));
                NormalItem item = new NormalItem();
    ...
    private List<NormalItem.eNormalType> GetSurroundingItemTypes(int x, int y)
    {
        // Tạo danh sách để lưu các loại mục tiêu xung quanh ô
        List<NormalItem.eNormalType> surroundingTypes = new List<NormalItem.eNormalType>();

        // Kiểm tra ô bên phải của ô hiện tại
        if (x + 1 < boardSizeX && !m_cells[x + 1, y].IsEmpty)
            surroundingTypes.Add((m_cells[x + 1, y].Item as NormalItem)?.ItemType ?? NormalItem.eNormalType.TYPE_ONE);

        // Kiểm tra ô bên trái của ô hiện tại
        if (x - 1 >= 0 && !m_cells[x - 1, y].IsEmpty)
            surroundingTypes.Add((m_cells[x - 1, y].Item as NormalItem)?.ItemType ?? NormalItem.eNormalType.TYPE_ONE);

        // Kiểm tra ô phía trên của ô hiện tại
        if (y + 1 < boardSizeY && !m_cells[x, y + 1].IsEmpty)
            surroundingTypes.Add((m_cells[x, y + 1].Item as NormalItem)?.ItemType ?? NormalItem.eNormalType.TYPE_ONE);

        // Kiểm tra ô phía dưới của ô hiện tại
        if (y - 1 >= 0 && !m_cells[x, y - 1].IsEmpty)
            surroundingTypes.Add((m_cells[x, y - 1].Item as NormalItem)?.ItemType ?? NormalItem.eNormalType.TYPE_ONE);

        // Trả về danh sách các loại mục tiêu xung quanh ô
        return surroundingTypes;
    }

    private NormalItem.eNormalType GetNewItemType(List<NormalItem.eNormalType> surroundingTypes)
    {
        // Lấy tất cả các loại mục tiêu có thể.
        List<NormalItem.eNormalType> allItemTypes = Enum.GetValues(typeof(NormalItem.eNormalType)).Cast<NormalItem.eNormalType>().ToList();

        // Loại bỏ những loại mục tiêu đã tồn tại trong những ô xung quanh.
        foreach (NormalItem.eNormalType type in surroundingTypes)
        {
            allItemTypes.Remove(type);
        }

        // Nếu tất cả các loại mục tiêu đều đã có trong những ô xung quanh, trả về một loại ngẫu nhiên.
        if (allItemTypes.Count == 0)
        {
            return Utils.GetRandomNormalType();
        }

        // Ngược lại, trả về loại mục tiêu đầu tiên từ những loại còn lại (loại ít gặp nhất).
        return allItemTypes[0];
    }
